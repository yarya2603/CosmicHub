...
local Tabs = {
    ...
    Games = Window:AddTab({ Title = "Игры", Icon = "gamepad-2"}),
    Settings = Window:AddTab({ Title = "Настройки", Icon = "settings" }),
}

    Fluent:Notify({
        Title = "👾 Cosmic Hub 👾",
        Content = "Добро пожаловать в Cosmic Hub",
        SubContent = "", -- Optional
    ...
    Tabs.Games:AddButton({
        Title = "Dead Rails Hub (Без Ключа)",
        Description = "",
        Callback = function()
    ...
    Tabs.Games:AddButton({
        Title = "Dead Rails Hub by markk (Нет Ключа)",
        Description = "",
        Callback = function()
...
-- a script hub could have themes in a global folder
-- and game configs in a separate folder per game
InterfaceManager:SetFolder("CosmicHubRussian")
SaveManager:SetFolder("CosmicHubRussian/SaveConfig")
